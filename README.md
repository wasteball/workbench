...
当前发布的 `0.1.10` 是商店上线前的公开预览版。Chrome 用户可以[下载预览版](https://github.com/wasteball/workbench/releases/tag/v0.1.10)，按照页面中的安装说明提前体验。Workbench 只维护、验收和发布 Chrome 插件。
...
1. 在[发布页面](https://github.com/wasteball/workbench/releases/tag/v0.1.10)下载文件名包含 `chrome` 的 ZIP 文件。
2. 右键 ZIP 文件并选择“全部解压”。
...
更新预览版时不要先卸载旧版。把新版本解压到原来的文件夹，再在扩展页面点击 Workbench 的“重新加载”，原有偏好和文件库记录会保留。若必须卸载，请先在“设置 → 权限与隐私”中选择“备份偏好与文件库”；Chrome 会在卸载时清除插件本地数据。
...
不配置存储时，上传和在线分享保持关闭，不影响文档工作台的任何本地功能。连接信息只在“设置 → 云端分享”中维护一处。文件库中的重命名、分类、移除和清空只整理当前浏览器里的记录，不会静默改动或删除对象存储中的文件。
...
- 同一插件直接更新会保留本地数据；必须卸载时，可以先备份偏好、分类和文件库记录，恢复后再重新填写密钥。

详细规则见[隐私说明](PRIVACY.md)。需要使用阿里云 OSS 或 API 上传服务时，再阅读[存储连接指南](docs/storage-configuration.md)。

## 当前版本说明

`0.1.10` 是当前公开预览版，只提供 Chrome 安装包。尚未迁移的旧版能力记录在[功能迁移矩阵](docs/feature-migration.md)中。
