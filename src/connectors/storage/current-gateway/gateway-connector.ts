...
function uploadWithProgress(
  ...
  return new Promise((resolve, reject) => {
    ...
    form.append('cdn', String(profile.cdn));
    form.append('publicRead', String(profile.publicRead));
    ...
    request.addEventListener('load', () => {
      ...
      if (request.status < 200 || request.status >= 300) {
        reject(new StorageConnectorError(`上传失败（HTTP ${request.status}）。`, request.status === 401 || request.status === 403 ? 'permission-denied' : 'provider-error'));
        return;
      }
    ...
    request.addEventListener('error', () => reject(new StorageConnectorError('无法连接上传服务，请检查网络和地址。', 'network')));
